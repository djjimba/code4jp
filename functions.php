	...
	function ss_framework_register_required_plugins() {
		...
		$plugins = array(
			...
			array(
				...
				'required' => false,
				'source'   => SS_BASE_URL . 'functions/assets/plugins/wp-gmappity-easy-google-maps.zip'
			)
...
if ( !function_exists('ss_custom_password_form') ) {
	...
	function ss_custom_password_form() {
		...
		return '<form class="form" action="' . get_option('siteurl') . '/wp-pass.php" method="post">
					<label for="' . $password_ID . '"><h6>' . __('This project is password protected. To view it please enter your password below:', 'ss_framework') . '</h6></label>
					<p><input name="post_password" id="' . $password_ID . '" type="password" size="40" type="text"><br />
					<input name="Submit" value="' . __('Submit', 'ss_framework') . '" type="submit"></p>
				</form> ';
